...
public class Game extends Application
{
    ...
    public void start(Stage primaryStage)
    {
        ...
        scene.setOnKeyPressed(event ->
        {
            ...
            {
                case UP : newDir = Dir.UP ;
                    break ;

                case DOWN : newDir = Dir.DOWN ;
                    break ;

                case LEFT : newDir = Dir.LEFT ;
                    break ;

                case RIGHT : newDir = Dir.RIGHT ;
                    break ;
            }
        }) ;
    ...
    private void moveRed()
    {
        ...
        else if(checkForWalls(red_movingAt, redGhost.getX(), redGhost.getY()) == true && numOfTurns(red_movingAt, redGhost.getX(), redGhost.getY()) == 1)
    ...
    private void movePink()
    {
        ...
        else if(checkForWalls(pink_movingAt, pinkGhost.getX(), pinkGhost.getY()) == true && numOfTurns(pink_movingAt, pinkGhost.getX(), pinkGhost.getY()) == 1)
        {							// if the ghost runs to a dead end it goes in the direction opposite to its current direction
    ...
    private void moveOrange()
    {
        ...
        else if(checkForWalls(orange_movingAt, orangeGhost.getX(), orangeGhost.getY()) == true && numOfTurns(orange_movingAt, orangeGhost.getX(), orangeGhost.getY()) == 1)
        {							// if the ghost runs to a dead end it goes in the direction opposite to its current direction
    ...
    private void moveCyan()
    {
        ...
        else if(checkForWalls(cyan_movingAt, cyanGhost.getX(), cyanGhost.getY()) == true && numOfTurns(cyan_movingAt, cyanGhost.getX(), cyanGhost.getY()) == 1)
        {							// if the ghost runs to a dead end it goes in the direction opposite to its current direction
